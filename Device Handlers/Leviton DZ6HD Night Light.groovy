/**
 *  Copyright 2018 Leon Schwartz, based on code that is Copyright 2017 Jason Xia
 *
...
metadata {
    ...
    tiles(scale: 2) {
        multiAttributeTile(name: "switch", type: "lighting", width: 6, height: 4, canChangeIcon: true) {
            ...
            //Allows for step-up/step-down dimming
            tileAttribute("device.level", key: "VALUE_CONTROL") {
                attributeState "VALUE_UP", action:"levelUp"
                attributeState "VALUE_DOWN", action:"levelDown"
            }
            //Allows for old-fashioned dimmer slider
            tileAttribute("device.level", key: "SLIDER_CONTROL") {
                attributeState "level", label: '${currentValue} %', action: "switch level.setLevel"
            }
        }
		//Presets for low/med/high lighting levels
        ...
        //Local nightlight ability (one switch at a time)
        ...
        main("switch")
        details(["switch", "low", "medium", "high", "nightlightOn", "nightlightOff", "refresh"])
	}
...
def refresh() {
    ...
    /* LS do not refresh all the configs
    for (i in 1..8) {
